import { createCapability, createId, Databases, FeatureExtractor, z } from "@bitler/core";
...
const addDocumentCapability = createCapability({
  ...
  handler: async ({ input, container }) => {
    ...
    await db.transaction(async (trx) => {
      await trx('documents').insert(input.documents.map((document) => ({
      ...
      }))).onConflict(['id', 'knowledgeBaseId']).merge({
        updatedAt: new Date().toISOString(),
      ...
      await trx('chunks')
        ...
        .whereIn('documentId', input.documents.map((document) => document.documentId))
        .del();
